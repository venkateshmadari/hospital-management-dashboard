...
interface AppointmentsPageProps {
  appointments: TotalAppointmentTypes[];
  loading: boolean;
  error: string | null;
  currentPage: number;
  totalPages: number;
  ...
  handleEditDoctor: (data: TotalAppointmentTypes) => void;
}
...
const AppointmentsPage = ({
  ...
  const allSelected = useMemo(
    () =>
      ...
      selectedAppointment?.length === appointments?.length,
    [appointments?.length, selectedAppointment?.length]
  );

  const handleSelectAll = (checked: boolean) => {
    if (checked) {
      setSelectedAppointment(appointments.map((doc) => doc.id));
    } else {
      setSelectedAppointment([]);
    }
  };
  ...
  const handleSelectDoctor = (id: string, checked: boolean) => {
    if (checked) {
      setSelectedAppointment((prev) => [...prev, id]);
    } else {
      setSelectedAppointment((prev) =>
        prev.filter((appointId) => appointId !== id)
      );
    }
  };
  return (
